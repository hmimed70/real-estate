 ...
 if (isset($_POST['submit'])) {
  ...
  if (strlen($_POST["phone"]) >12 || $_POST['phone'] < 6) {
   ...
   if(empty($emailErr) && empty($passErr) && empty($passConfirmErr) && empty($phoneErr) && empty($nameErr)) {
    ...
    //hash password
    $pass = md5($pass);  
    $check_email = mysqli_query($conn, "SELECT email FROM admin where email = '$email' ");
    if(mysqli_num_rows($check_email) > 0){
     $emailErr = "Email already exist" ;
    }
    //if email not exist create the user account
    if(empty($emailErr)) {
        $query = "INSERT INTO admin (fullName,email,pass_admin,phone) ";
        $query.=" VALUES ('$name','$email','$pass','$phone')";
                ...
                <div class="col-md-12 login-form-1 " style="float:none;margin:auto;">
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group ">
                                <span> Already have account ? <a href="login_admin.php" class="signup-link">login here</a>
                            </div>
                        </div>
                    </form>
                </div>
